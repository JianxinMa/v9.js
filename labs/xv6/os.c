...
enum {
    ...
    NINODE = 50, // maximum number of active i-nodes  XXX make this more
                 // dynamic ...
...
struct inode
    inode[NINODE]; // inode cache XXX make dynamic and eventually power
                   // of 2, look into iget()
...
char* kalloc()
{
    ...
    if (r = mem_free)
        ...
        mem_top
            += PAGE; // XXX uint issue is going to be a problem with other
                     // pointer compares!
    else
...
int consoleread(struct inode* ip, char* dst, int n)
{
    ...
    while (n > 0) {
        ...
        *dst++ = c; // XXX pagefault possible in cli (perhaps use inode
                    // locks to
...
// fake IDE disk; stores blocks in memory.  useful for running kernel
// without
// scratch disk.  XXX but no good for stressing demand pageing logic!
ideinit() { memdisk = P2V + (mem_sz - FSSIZE); }

// sync buf with disk.  if B_DIRTY is set, write buf to disk, clear B_DIRTY,
// set
// B_VALID.
...
// * B_DIRTY: the buffer data has been modified and needs to be written to
// disk.
binit()
{
    struct buf* b;
...
// (the type, device numbers, and data size) along with a list of blocks
// where
// the data can be found.
//
// The kernel keeps a cache of the in-use on-disk structures to provide a
// place
...
// represented by the I_BUSY flag in the in-memory copy.  Because inode
// locks
// are held during disk accesses,
// they are implemented using a flag rather than with spin locks.  Callers
// are
...
// return pointers to *unlocked* inodes.  It is the callers' responsibility
// to
// lock them before using them.
// A non-zero ip->ref keeps these unlocked inodes in the cache.

// find the inode with number inum and return the in-memory copy.  does not
// lock
...
// Return the disk block address of the nth block in inode ip. If there is
// no
...
// only called when the inode has no links to it (no directory entries
// referring
// to it)
...
// look for a directory entry in a directory. If found, set *poff to byte
// offset
// of entry.
struct inode* dirlookup(struct inode* dp, char* name, uint* poff)
{
    ...
    for (off = 0; off < dp->size; off += sizeof(de)) {
        if (readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
            panic("dirlink read");
        if (de.d_ino
            && !namecmp(name, de.d_name)) { // entry matches path element
            if (poff)
                *poff = off;
...
// return the inode for the parent and copy the final path element into
// name,
...
int pipewrite(struct pipe* p, char* addr, int n)
{
    ...
    for (i = 0; i < n; i++) {
        while (
            p->nwrite == p->nread + PIPESIZE) { // XXX DOC: pipewrite-full
        ...
        p->data[p->nwrite++ % PIPESIZE] = addr[i]; // XXX pagefault possible
                                                   // in cli (use inode
                                                   // locks instead?)
    }
...
int piperead(struct pipe* p, char* addr, int n)
{
    ...
    for (i = 0; i < n; i++) { // XXX DOC: piperead-copy
        if (p->nread == p->nwrite)
            break;
        addr[i] = p->data[p->nread++
            % PIPESIZE]; // XXX pagefault possible in cli
...
// int cyc() { asm(CYC); } XXX
int sockrx(
    int sd, void* p, int n) // XXX this is always going to be slow until
                            ...
                            // delta))
{
    ...
    while (n > 0) {
        while (!sockpoll(sd))
            ssleep(1); // XXX needs to block properly XXX should I lock the
                       // inode?
        //    while (!sockpoll(sd)) { printf("x"); ssleep(1); } // XXX needs
        //    to
        //    block properly XXX should I lock the inode?
        //    while (!sockpoll(sd)) { cy = cyc(); for (r = 0; r < 10; r++) {
        //    nyc =
        //    cyc(); printf("[%d]",nyc - cy); cy = nyc; ssleep(100); } } //
        //    XXX
        //    needs to block properly XXX should I lock the inode?
        //    while (!sockpoll(sd)) { cy = cyc(); for (r = 0; r < 10; r++) {
        //    nyc =
        //    cyc(); printf("[%d]",nyc - cy); cy = nyc; ssleep(1); } } //
        //    XXX needs
...
int read(int fd, char* addr, int n)
{
    ...
    case FD_RFS:
        ...
        if (socktx(f->off, h, 8) || sockrx(f->off, &r, 4)
            ...
            return -1; // XXX need to zortch the file  is this correct for
                       // zero?
        return r;
    }
    panic("read");
...
uint htonl(uint a)
{
    return (a >> 24) | ((a >> 8) & 0xff00) | ((a << 8) & 0xff0000)
        | (a << 24);
} // XXX eliminate
...
int open(char* path, int oflag) // XXX, int mode)
{
    ...
    if (!memcmp(path, "rfs/", 4)) {
        ...
        f = getf(fd);
        if (sockconnect(
                f->off, AF_INET | (htons(5003) << 16), htonl(0x7f000001)))
            return -1; // XXX close?
        f->type = FD_RFS;
        h[0] = M_OPEN;
        h[1] = oflag;
...
// Exit the current process.  Does not return.  An exited process remains in
// the
// zombie state
// until its parent calls wait() to find out it exited.  Special treatment
// for
// process 0 and 1.
exit(int rc)
{
    ...
    if (u->pid == 0) {
        ...
        panic("exit() init exiting"); // XXX reboot after all processes go
                                      // away?
...
// grow process by n bytes             XXX need to verify that u->sz is
// always
...
userinit()
{
    ...
    *(uint*)(mem + 16) = 0xff2016ff;
    ...
    mem = memcpy(
        mem + 20, (char*)init_start, (uint)userinit - (uint)init_start);
...
setupkvm()
{
    ...
    kpdir = memset(
        kalloc(), 0, PAGE); // kalloc returns physical addresses here
...
// return the address of the PTE in page table pd that corresponds to
// virtual
// address va
uint* walkpdir(uint* pd, uint va)
{
...
mappage(uint* pd, uint va, uint pa, int perm)
{
    ...
    if (*(pde = &pd[va >> 22]) & PTE_P)
        ...
        *pde = (V2P + (uint)(pt = memset(kalloc(), 0, PAGE))) | PTE_P
            | PTE_W | PTE_U;
    pte = &pt[(va >> 12) & 0x3ff];
    if (*pte & PTE_P) {
        printf("*pte=0x%x pd=0x%x va=0x%x pa=0x%x perm=0x%x", *pte, pd, va,
            pa, perm);
        panic("remap");
    }
    *pte = pa | perm;
    ...
    int create) // XXX rename grow() ?
{
    ...
    if (newsz <= oldsz)
        panic("allocuvm: newsz <= oldsz"); // XXX do pre-checking in caller,
                                           // no more
    ...
    while (va < newsz) {
        if (create)
            mappage(pd, va, V2P + (memset(kalloc(), 0, PAGE)),
                PTE_P | PTE_W | PTE_U);
        else
...
// oldsz and newsz need not be page-aligned, nor does newsz need to be less
// than
// oldsz.   XXXX wha why?
// oldsz can be larger than the actual process size.  Returns the new
// process
// size.
int deallocuvm(
    uint* pd, uint oldsz, uint newsz) // XXX rename shrink() ?? //XXX
                                      ...
                                      // if present !!!
{
    ...
    while (va < oldsz) {
        ...
        if (*pde & PTE_P) { // XXX this may no longer be true if we are
                            // paging out
                            // pde/pte's?
            pt = P2V + (*pde & -PAGE);
...
freevm(uint* pd)
{
    ...
    for (i = 0; i < ((USERTOP >> 22) & 0x3ff);
        ...
        if (pd[i] & PTE_P)
            kfree(
                P2V + (pd[i] & -PAGE)); // deallocate all page table entries
    }
...
uint* copyuvm(uint* pd, uint sz)
{
    ...
    for (va = 0; va < sz; va += PAGE) {
        ...
        if (*pte & PTE_P)
            mappage(d, va,
                V2P + (memcpy(kalloc(), P2V + (*pte & -PAGE), PAGE)),
...
trap(uint* sp, double g, double f, int c, int b, int a, int fc, uint* pc)
{
    ...
    case FSYS + USER:
        ...
        default:
            printf(
                "pid:%d name:%s unknown syscall %d\n", u->pid, u->name, a);
            a = -1;
            break;
        }
        if (u->killed)
