...
int vsprintf(char* s, char* f, va_list v)
{
    ...
    while (c = *f++) {
        ...
        case 'f':
            ...
            p = b + BUFSIZ - 1;
            i = prec;
            //            while (i >= 0 || d > 0.0) { if (!i-- && prec) *--p
            //            = '.';
            //            *--p = '0' + ((d > 1.0e15) ? 0 :
            //            ((int)fmod(d+0.5,10.0))); d
            //            = floor(d * 0.1); }  XXX
            while (i >= 0 || d > 0.0) {
                if (!i-- && prec)
                    *--p = '.';
                *--p = '0' + ((d > 1000000000000000.0)
                                     ? 0
                                     : ((int)fmod(d + 0.5, 10.0)));
                d = floor(d * 0.1);
            }
        ...
        case 'G':
            ...
            i = 3;
            break; // XXX
        //    case 'e': case 'E': e1: p = "<e>"; i = 3; break; // XXX see
        //    above 'f'
        default:
            *e++ = c;
            continue;
...
int printf(char* f, ...)
{
    ...
    va_list v;
    va_start(v, f);
    return write(1, s, vsprintf(s, f, v));
}
...
int dprintf(int d, char* f, ...)
{
    ...
    va_list v;
    va_start(v, f);
    return write(d, s, vsprintf(s, f, v));
}
