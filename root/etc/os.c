...
enum {
  ...
  NINODE =
      50,    // maximum number of active i-nodes  XXX make this more dynamic ...
...
struct trapframe { // layout of the trap frame built on the stack by trap
                   // handler
  int sp, pad1;
  double g, f;
  int c, pad2;
...
enum {
  B_BUSY = 1,  // buffer is locked by some process
...
struct direct { // directory is a file containing a sequence of direct
                // structures.
  uint d_ino;
  char d_name[DIRSIZ];
};
...
struct buf bfreelist;       // linked list of all buffers, through prev/next.
                            // bfreelist.next is most recently used
struct inode inode[NINODE]; // inode cache XXX make dynamic and eventually power
                            // of 2, look into iget()
...
xstrncpy(char *s, char *t,
         int n) // no return value unlike strncpy XXX remove me only called once
{
  while (n-- > 0 && (*s++ = *t++))
    ;
  while (n-- > 0)
...
safestrcpy(char *s, char *t,
           int n) // like strncpy but guaranteed to null-terminate.
{
  if (n <= 0)
    return;
  while (--n > 0 && (*s++ = *t++))
...
char *kalloc() {
  ...
  if (r = mem_free)
    ...
    mem_top += PAGE; // XXX uint issue is going to be a problem with other
                     // pointer compares!
  else
    panic("kalloc failure!"); // XXX need to sleep here!
  splx(e);
  return r;
}
...
kfree(char *v) {
  int e = splhi();
  if ((uint)v % PAGE || v < (char *)(P2V + kreserved) ||
      (uint)v >= P2V + (mem_sz - FSSIZE))
    panic("kfree");
  *(char **)v = mem_free;
  mem_free = v;
  splx(e);
...
int consoleread(struct inode *ip, char *dst, int n) {
  ...
  e = splhi();
  while (n > 0) {
    if (input.r == input.w && n < target)
      break; // block until at least one byte transfered
    ...
    *dst++ = c; // XXX pagefault possible in cli (perhaps use inode locks to
...
// fake IDE disk; stores blocks in memory.  useful for running kernel without
// scratch disk.  XXX but no good for stressing demand pageing logic!
ideinit() { memdisk = P2V + (mem_sz - FSSIZE); }

// sync buf with disk.  if B_DIRTY is set, write buf to disk, clear B_DIRTY, set
// B_VALID.
...
// disk block contents.  Caching disk blocks.
...
// * B_BUSY: the block has been returned from bread and has not been passed back
// to brelse.
...
// structure holds metadata
// (the type, device numbers, and data size) along with a list of blocks where
// the data can be found.
//
// The kernel keeps a cache of the in-use on-disk structures to provide a place
// for synchronizing access
// to inodes shared between multiple processes.
//
// ip->ref counts the number of pointer references to this cached inode;
// references are typically kept in
// struct file and in u->cwd.  When ip->ref falls to zero, the inode is no
// longer cached.  It is an error
// to use an inode without holding a reference to it.
//
// Processes are only allowed to read and write inode metadata and contents when
// holding the inode's lock,
// represented by the I_BUSY flag in the in-memory copy.  Because inode locks
// are held during disk accesses,
// they are implemented using a flag rather than with spin locks.  Callers are
// responsible for locking
// inodes before passing them to routines in this file; leaving this
// responsibility with the caller makes
...
// file that return inode pointers
// return pointers to *unlocked* inodes.  It is the callers' responsibility to
// lock them before using them.
// A non-zero ip->ref keeps these unlocked inodes in the cache.

// find the inode with number inum and return the in-memory copy.  does not lock
...
iunlock(struct inode *ip) {
  int e = splhi();
  if (!ip || !(ip->flags & I_BUSY) || ip->ref < 1)
    panic("iunlock");
...
// blocks on the disk.
// The first NDIR blocks are listed in ip->dir[].  The next NIDIR blocks are
// listed in the block ip->idir[].
...
uint bmap(struct inode *ip, uint bn) {
  ...
  if (!(addr = ip->idir[bn / 1024]))
    ip->idir[bn / 1024] = addr = balloc();
  bp = bread(addr);
  a = (uint *)bp->data;
  if (!(addr = a[bn & 1023])) {
...
// only called when the inode has no links to it (no directory entries referring
// to it)
// and has no in-memory reference to it (is not an open file or current
// directory)
...
int readi(struct inode *ip, char *dst, uint off, uint n) {
  ...
  if ((ip->mode & S_IFMT) == S_IFCHR) { // S_IFBLK ??
    ...
    if (ip->dir[0] >= NDEV || !devsw[ip->dir[0]].read)
      return -1; // XXX refactor
...
int writei(struct inode *ip, char *src, uint off, uint n) {
  ...
  if ((ip->mode & S_IFMT) == S_IFCHR) { // XXX S_IFBLK ??
    ...
    if (ip->dir[0] >= NDEV || !devsw[ip->dir[0]].write)
      return -1; // XXX refactor
    return devsw[ip->dir[0]].write(ip, src, n);
  }
  if (off > ip->size || off + n < off)
    return -1;
  if (off + n > (NDIR + NIDIR * 1024) * PAGE)
    return -1;
...
// look for a directory entry in a directory. If found, set *poff to byte offset
// of entry.
...
// element following the copied one.
// The returned path has no leading slashes, so the caller can check *path=='\0'
// to see if the name is the last one.
...
// which must have room for DIRSIZ bytes.
...
int pipewrite(struct pipe *p, char *addr, int n) {
  ...
  for (i = 0; i < n; i++) {
    ...
    p->data[p->nwrite++ % PIPESIZE] =
        addr[i]; // XXX pagefault possible in cli (use inode locks instead?)
  }
...
int piperead(struct pipe *p, char *addr, int n) {
  ...
  for (i = 0; i < n; i++) { // XXX DOC: piperead-copy
    if (p->nread == p->nwrite)
      break;
    addr[i] = p->data[p->nread++ % PIPESIZE]; // XXX pagefault possible in cli
                                              // (use inode locks instead?)
  }
...
struct inode *create(char *path, ushort mode, int dev) {
  ...
  if (ip = dirlookup(dp, name, 0)) {
    iunlockput(dp);
    ilock(ip);
    if ((mode & S_IFMT) == S_IFREG && (ip->mode & S_IFMT) == S_IFREG)
      return ip;
    iunlockput(ip);
    return 0;
  }
  ...
  if ((mode & S_IFMT) == S_IFDIR) { // create . and .. entries
    ...
    if (dirlink(ip, ".", ip->inum) || dirlink(ip, "..", dp->inum))
      panic("create dots");
  }
...
struct file *getf(uint fd) {
  return (fd < NOFILE) ? u->ofile[fd] : 0;
}

int sockopen(int family, int type, int protocol) {
  asm(LL, 8);
  asm(LBL, 16);
  asm(LCL, 24);
  asm(NET1);
...
int sockconnect(int fd, uint family_port, uint addr) {
  asm(LL, 8);
  asm(LBL, 16);
  asm(LCL, 24);
  asm(NET3);
}
int sockread(int sd, char *addr, int n) {
  asm(LL, 8);
  asm(LBL, 16);
  asm(LCL, 24);
  asm(NET4);
}
int sockwrite(int sd, char *addr, int n) {
  asm(LL, 8);
  asm(LBL, 16);
  asm(LCL, 24);
  asm(NET5);
...
int socktx(int sd, void *p, int n) {
  int r;
  while (n > 0) {
    if ((r = sockwrite(sd, p, n)) <= 0)
      return -1; // XXX need some kind of ready to write? XXX <= 0?
    n -= r;
    p += r;
  }
  return 0;
...
// int cyc() { asm(CYC); } XXX
int sockrx(int sd, void *p, int n) // XXX this is always going to be slow until
                                   // I fix the sockpoll (since ssleep waits for
                                   // a comparitively long time (timer vs. em
                                   // delta))
{
  int r; // uint cy, nyc;
  while (n > 0) {
    while (!sockpoll(sd))
      ssleep(1); // XXX needs to block properly XXX should I lock the inode?
    //    while (!sockpoll(sd)) { printf("x"); ssleep(1); } // XXX needs to
    //    block properly XXX should I lock the inode?
    //    while (!sockpoll(sd)) { cy = cyc(); for (r = 0; r < 10; r++) { nyc =
    //    cyc(); printf("[%d]",nyc - cy); cy = nyc; ssleep(100); } } // XXX
    //    needs to block properly XXX should I lock the inode?
    //    while (!sockpoll(sd)) { cy = cyc(); for (r = 0; r < 10; r++) { nyc =
    //    cyc(); printf("[%d]",nyc - cy); cy = nyc; ssleep(1); } } // XXX needs
    //    to block properly XXX should I lock the inode?
    if ((r = sockread(sd, p, n)) <= 0) {
      printf("sockrx() sockread()\n");
      return -1;
    } //  XXX <= 0?
    n -= r;
    p += r;
...
int fstat(int fd, struct stat *st) {
  int r;
  struct file *f;
  if (!(f = getf(fd)) || !mvalid(st, sizeof(struct stat)))
    return -1;
  switch (f->type) {
  case FD_INODE:
    ilock(f->ip);
  ...
  case FD_RFS:
    r = M_FSTAT;
    if (socktx(f->off, &r, 4) || sockrx(f->off, &r, 4) ||
        (r == 0 && sockrx(f->off, st, sizeof(struct stat))))
      return -1;
    return r;
  }
  return -1;
...
int read(int fd, char *addr, int n) {
  ...
  int h[2];
  struct file *f;
  if (!(f = getf(fd)) || !f->readable || !mvalid(addr, n))
    return -1;
  switch (f->type) {
  case FD_PIPE:
    return piperead(f->pipe, addr, n);
  case FD_SOCKET:
    while (!sockpoll(f->off))
      ssleep(1); // XXX needs to block properly XXX should I lock the inode?
                 // (right now there isn't an inode!)
  ...
  case FD_RFS:
    ...
    h[1] = n;
    if (socktx(f->off, h, 8) || sockrx(f->off, &r, 4) ||
        ((uint)r <= n && sockrx(f->off, addr, r)))
      return -1; // XXX need to zortch the file  is this correct for zero?
    return r;
  }
  panic("read");
}
int write(int fd, char *addr, int n) {
  int r, h[2];
  struct file *f;
  if (!(f = getf(fd)) || !f->writable || !mvalid(addr, n))
    return -1;
  switch (f->type) {
  case FD_PIPE:
    return pipewrite(f->pipe, addr, n);
  case FD_SOCKET:
    return sockwrite(f->off, addr, n); // XXX needs to block
  case FD_INODE:
    ilock(f->ip);
  ...
  case FD_RFS:
    ...
    h[1] = n;
    if (socktx(f->off, h, 8) || (n > 0 && socktx(f->off, addr, n)))
      return -1; // XXX need to zortch the file
    return n;
  }
  panic("write");
}
...
int lseek(int fd, int offset, uint whence) {
  ...
  case FD_RFS:
    ...
    h[1] = offset;
    h[2] = whence;
    if (socktx(f->off, h, 12) || sockrx(f->off, &r, 4))
      return -1;
    return r;
  }
  return -1;
...
int link(char *old, char *new) {
  ...
  if (!svalid(old) || !svalid(new) || !(ip = namei(old)))
    return -1;
  ilock(ip);
  if ((ip->mode & S_IFMT) == S_IFDIR) {
    iunlockput(ip);
...
int unlink(char *path) {
  ...
  if (!svalid(path) || !(dp = nameiparent(path, name)))
    return -1;
  ilock(dp);
...
int open(char *path, int oflag) // XXX, int mode)
{
  ...
  if (!memcmp(path, "rfs/", 4)) {
    ...
    if ((fd = socket(AF_INET, SOCK_STREAM, 0)) < 0)
      return -1; // XXX reuse?
    f = getf(fd);
    if (sockconnect(f->off, AF_INET | (htons(5003) << 16), htonl(0x7f000001)))
      return -1; // XXX close?
    f->type = FD_RFS;
    h[0] = M_OPEN;
    h[1] = oflag;
    h[2] = 0; // mode;
    h[3] = strlen(path) + 1;
    if (socktx(f->off, h, 16) || socktx(f->off, path, h[3]) ||
        sockrx(f->off, &r, 4) || r < 0)
      return -1; // XXX sockclose?
    return fd;
  } else if (oflag & O_CREAT) {
    if (!(ip = create(path, S_IFREG, 0)))
      return -1;
  } else {
    if (!(ip = namei(path)))
      return -1;
...
int mkdir(char *path) {
  struct inode *ip;
  if (!svalid(path) || !(ip = create(path, S_IFDIR, 0)))
    return -1;
  iunlockput(ip);
  return 0;
}
...
int mknod(char *path, int mode, int dev) {
  struct inode *ip;
  if (!svalid(path) || !(ip = create(path, mode, dev)))
    return -1;
  iunlockput(ip);
  return 0;
}
...
int exec(char *path, char **argv) {
  ...
  if (!svalid(path))
    return -1;
  for (argc = 0;; argc++) {
    if (argc >= MAXARG || !mvalid(argv + argc, 4))
      return -1;
    if (!argv[argc])
      break;
    if (!svalid(argv[argc]))
  ...
  if (hdr.magic != 0xC0DEF00D)
    goto bad; // XXX some more hdr checking?
  ...
  if (!(sz = allocuvm(pd, 0, ip->size, 1)))
    goto bad;

  for (i = 0; i < sz; i += PAGE) {
    if (!(pte = walkpdir(pd, i)))
      panic("exit() address should exist");
    n = (sz - i < PAGE) ? sz - i : PAGE;
    if (readi(ip, P2V + (*pte & -PAGE), i, n) != n)
      goto bad;
  }
  ...
  if (!(sz = allocuvm(pd, sz, sz + hdr.bss + STACKSZ, 0)))
    goto bad;

  // initialize the top page of the stack
  sz &= -PAGE;
  mappage(pd, sz, V2P + (sp = memset(kalloc(), 0, PAGE)),
          PTE_P | PTE_W | PTE_U);
...
// Exit the current process.  Does not return.  An exited process remains in the
// zombie state
// until its parent calls wait() to find out it exited.  Special treatment for
// process 0 and 1.
exit(int rc) {
  struct proc *p;
  int fd;
  ...
  if (u->pid == 0) {
    for (;;)
      asm(IDLE);
  } // spin in the arms of the kernel (cant be paged out)
  else if (u->pid == 1)
    panic("exit() init exiting"); // XXX reboot after all processes go away?
...
// Kill the process with the given pid.  Process won't exit until it returns to
// user space (see trap()).
int kill(int pid) {
  struct proc *p;
  int e = splhi();
...
// grow process by n bytes             XXX need to verify that u->sz is always
// at a 4 byte alignment  !!!!!
int sbrk(int n) {
  uint osz, sz;
  if (!n)
    return u->sz;
  ...
  if (n > 0) {
    ...
    if (!(sz = deallocuvm(u->pdir, sz, sz + n)))
      return -1;
    pdir(V2P + (uint)(u->pdir));
  }
  u->sz = sz;
...
int socket(int family, int type, int protocol) {
  ...
  if ((sd = sockopen(family, type, protocol)) < 0) {
    u->ofile[fd] = 0;
    fileclose(f);
    return sd;
  }
...
int poll(struct pollfd *pfd, uint n, int msec) {
  ...
  struct file *f;
  struct pollfd *p, *pn;
  if (n && !mvalid(pfd, n * sizeof(struct pollfd)))
    return -1;
  pn = &pfd[n];
  for (;;) {
    r = 0;
    for (p = pfd; p != pn; p++) {
      ...
      if (!(f = getf(p->fd)))
        ...
        switch (f->type) {
        case FD_PIPE:
          if (f->pipe->nwrite != f->pipe->nread || !f->pipe->writeopen)
            ev = POLLIN;
          break;
        case FD_SOCKET:
          if (sockpoll(f->off))
    ...
    ssleep(1);
    if (msec > 100)
      msec -= 100; // XXX this wrongly assumes one tick equals 100ms
    else if (msec > 0)
      msec = 0;
  }
  return r;
...
int connect(int fd, uint *addr, int addrlen) {
  struct file *f;
  if (!(f = getf(fd)) || addrlen < 8 || !mvalid(addr, addrlen))
    return -1;
  return sockconnect(f->off, addr[0], addr[1]);
}

int sockbind(int fd, uint family_port, uint addr) {
  asm(LL, 8);
  asm(LBL, 16);
  asm(LCL, 24);
  asm(NET7);
...
int accept(int fd, uint *addr,
           int *addrlen) // XXXX params!!!! accept(int,*|0,*|0)
{
  int sd;
  struct file *f;
  if (!(f = getf(fd)))
  ...
  if ((sd = sockaccept(f->off, 0, 0)) < 0)
    return sd; // XXX null params for now

  if (!(f = filealloc()) ||
      (fd = fdalloc(f)) < 0) { // XXX do this before sockaccept?
    if (f)
      fileclose(f);
    return -1;
  }
...
// Look in the process table for an UNUSED proc.  If found, change state to
// EMBRYO and initialize
...
setupkvm() {
  ...
  kpdir = memset(kalloc(), 0, PAGE); // kalloc returns physical addresses here
                                     // (kfree wont work until later on)
...
// return the address of the PTE in page table pd that corresponds to virtual
// address va
uint *walkpdir(uint *pd, uint va) {
...
mappage(uint *pd, uint va, uint pa, int perm) {
  ...
  if (*(pde = &pd[va >> 22]) & PTE_P)
    ...
    *pde =
        (V2P + (uint)(pt = memset(kalloc(), 0, PAGE))) | PTE_P | PTE_W | PTE_U;
  pte = &pt[(va >> 12) & 0x3ff];
  if (*pte & PTE_P) {
    printf("*pte=0x%x pd=0x%x va=0x%x pa=0x%x perm=0x%x", *pte, pd, va, pa,
           perm);
    panic("remap");
  }
  *pte = pa | perm;
...
int allocuvm(uint *pd, uint oldsz, uint newsz,
             int create) // XXX rename grow() ?
{
  uint va;
  if (newsz > USERTOP)
    return 0; // XXX make sure this never happens...
  if (newsz <= oldsz)
    panic("allocuvm: newsz <= oldsz"); // XXX do pre-checking in caller, no more
                                       // post-checking needed
...
// oldsz and newsz need not be page-aligned, nor does newsz need to be less than
// oldsz.   XXXX wha why?
// oldsz can be larger than the actual process size.  Returns the new process
// size.
int deallocuvm(uint *pd, uint oldsz, uint newsz) // XXX rename shrink() ?? //XXX
                                                 // memset 0 top of partial page
                                                 // if present !!!
{
  uint va, *pde, *pte, *pt;

  if (newsz >= oldsz)
    return oldsz; // XXX maybe make sure this never happens
  ...
  while (va < oldsz) {
    ...
    if (*pde & PTE_P) { // XXX this may no longer be true if we are paging out
                        // pde/pte's?
      pt = P2V + (*pde & -PAGE);
...
freevm(uint *pd) {
  ...
  if (!pd)
    panic("freevm: no pd");
  deallocuvm(pd, USERTOP,
             0); // deallocate all user memory XXX do this more simply
  for (i = 0; i < ((USERTOP >> 22) & 0x3ff);
       i++) { // for (i = 0; i < PDX(USERTOP); i++)
    if (pd[i] & PTE_P)
      kfree(P2V + (pd[i] & -PAGE)); // deallocate all page table entries
  }
...
uint *copyuvm(uint *pd, uint sz) {
  ...
  for (va = 0; va < sz; va += PAGE) {
    if (!(pte = walkpdir(pd, va)))
      panic("copyuvm: pte should exist");

    if (*pte & PTE_P)
      mappage(d, va, V2P + (memcpy(kalloc(), P2V + (*pte & -PAGE), PAGE)),
              PTE_P | PTE_W | PTE_U); // XXX implement copy on write
    else
...
found:
  ...
  // else printf("spin(%d)\n",u->pid);    XXX else do a wait for interrupt?
  // (which will actually pend because interrupts are turned off here)
}
...
trap(uint *sp, double g, double f, int c, int b, int a, int fc, uint *pc) {
  ...
  case FSYS + USER:
    ...
    case S_connect:
      ...
      break;
    default:
      printf("pid:%d name:%s unknown syscall %d\n", u->pid, u->name, a);
      a = -1;
      break;
    }
    if (u->killed)
  ...
  case FRPAGE + USER: // XXX
    ...
    mappage(u->pdir, va & -PAGE, V2P + (memset(kalloc(), 0, PAGE)),
            PTE_P | PTE_W | PTE_U);
    return;
...
main() {
  ...
  setupkvm();
  kpdir[0] = kpdir[(uint)USERTOP >>
                   22]; // need a 1:1 map of low physical memory for awhile
