...
//   -o  Create executable file and terminate normally.  If -o and -s are
//   omitted,
  ...
  for (;;) {
    ...
    case '#':
      if (!memcmp(pos, "include", 7)) {
        ...
        while (*pos && *pos != '>' && *pos != '"' && b < sizeof(iname) - 1) {
          iname[b++] = *pos++;
        }
        iname[b] = 0;
        if (stat(iname, &st)) {
      ...
      while ((b = *pos++) != tk) {
        if (b == '\\') {
          ...
          case '\n':
            ...
            continue;
          case 'x':
            //            b = (*pos - '0') * 16 + pos[1] - '0'; pos += 2; // XXX
            //            this is broke!!! 0xFF needs to become -1 also
            switch (*pos) {
            case '0' ... '9':
              b = *pos++ - '0';
              break;
            ...
            default:
              b = 0;
              pos++;
              break; // XXX you can try a few in a reg c compiler?!
            }
            switch (*pos) {
            case '0' ... '9':
              b = b * 16 + *pos++ - '0';
    ...
    case '.':
      ...
      return;
    // XXX eventually test for float? is this structure access x.y or floating
    // point .5?
...
int imm() /// XXX move these back down once I validate prototypes working for
/// double immf()
{
  int *b = e, c;
  expr(Cond);
...
int tinc(uint t) { // XXX return unsigned?
  if (t & PMASK) {
    ...
    return tsize(
        ((array_t *)(va + (t >> TSHIFT)))->type); // XXX need to test this!
  else {
    return 1;
  }
}
  ...
  if (tk == Paren) { // function
    ...
    for (pt = p = 0; tk != ')'; p++) {
      ...
      if ((a = basetype())) {
        ...
        d = 0;
        type(&d, &n, a); // XXX should accept both arg/non arg if v == 0  XXX
      ...
      if (n) {
        ...
        if (*v == n) {
          *v = (ident_t *)ploc; // hack if function name same as parameter
        }
        ploc->class = n->class; // XXX make sure these are unwound for abstract
      ...
      if (tk == Comma) {
        next(); // XXX desparately need to flag an error if not a comma or close
      }
      // paren!
      if (tk == Dots) {
        next();
    ...
    *(uint *)(vp + 4) = pt;
    ...
    while (tk == Brak) { // array
      ...
      a = 0;
      if (tk != ']') {
        // XXX need constant in vc...not tree!  I think this is going go be a
    ...
    for (;;) {
      ...
      if (!v) {
        ...
        if (sc != Typedef) { // XXX typedefs local to functions?
          if ((t & TMASK) == ARRAY) {
            v->class = (sc == Auto) ? Lea : Leag; // not lvalue if array
          }
          size = tsize(t);
          align = talign(t);
          if (sc == Auto) {
            ...
            if (tk == Assign) {
              v->val = data =
                  (data + align - 1) & -align; // allocate data space
              hglo = data;
              next();
              if (tk == '"') {
                if ((t & TMASK) != ARRAY) {
                ...
                next();
                if (size) {
                  data = hglo + size * align; // XXX need to zero fill if size >
                }
...
void trim() { // trim dead code from expression statements (just the common
  // cases)
  if (*e >= Cic && *e <= Cus) {
  ...
  while (tk >= lev) {
    ...
    case Add:
      ...
      if ((t & PAMASK) && ty <= UINT) {
        ...
        if (*b == Numf && *d == Numf) {
          *e = Numf;
          *(double *)(e + 2) = *(double *)(b + 2) + *(double *)(d + 2);
        } else {
          nodc(Addf, b, d);
        }
        ty = DOUBLE;
    ...
    case Dot: // XXX do some optimization for x.y on stack or global, then work
    ...
    case Arrow:
      ...
      for (m = ((struct_t *)(va + (ty >> TSHIFT)))->member; m; m = m->next)
        if (m->id == id) {
          goto found;
        }
    ...
    case Brak: // XXX these dont quite work when used with pointers?  still?
      // test?
...
void opa(int *a, int o, int comm) {
  ...
  case Ptr:
    ...
    em(SX + smod(a[1])); // XXX many more (SX,imm) optimizations possible (here
    // and elsewhere)
    return;
  default:
    err("lvalue expected");
  ...
  case Assign:
    ...
    case Ptr:
      ...
      case Auto:
        ...
        break; // *loc = expr // XXX all good? in the presense of multiple
      // casts?
      case Static:
        rv(b);
...
void stmt() {
  ...
  case Switch:
    ...
    } else { // XXX lots of possible signed/unsigned under/overflow issues in
      // this block
      cmin = cmax = *(et = es - 2);
      while (et > e) {
        et -= 2;
  ...
  case Id:
    if (*pos == ':') {
      ...
      if (!id->class) {
        ...
        err("bad label name"); // XXX should let labels override globals just
      }
      // like other locals
      next();
      stmt();
...
int main(int argc, char *argv[]) {
  ...
  pos = "asm auto break case char continue default do double else enum float "
        ...
        "sizeof static struct switch typedef union unsigned void while va_list "
