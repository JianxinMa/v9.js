...
char ops[]
    = "HALT,ENT ,LEV ,JMP ,JMPI,JSR ,JSRA,LEA ,LEAG,CYC "
      ",MCPY,MCMP,MCHR,MSET,"
      "LL  ,LLS ,LLH ,LLC ,LLB ,LLD ,LLF ,LG  ,LGS ,LGH ,LGC ,LGB ,LGD "
      ",LGF ,"
      "LX  ,LXS ,LXH ,LXC ,LXB ,LXD ,LXF ,LI  ,LHI ,LIF ,LBL "
      ",LBLS,LBLH,LBLC,"
      "LBLB,LBLD,LBLF,LBG ,LBGS,LBGH,LBGC,LBGB,LBGD,LBGF,LBX "
      ",LBXS,LBXH,LBXC,"
      "LBXB,LBXD,LBXF,LBI ,LBHI,LBIF,LBA ,LBAD,SL  ,SLH ,SLB ,SLD ,SLF ,SG "
      " ,"
      "SGH ,SGB ,SGD ,SGF ,SX  ,SXH ,SXB ,SXD ,SXF "
      ",ADDF,SUBF,MULF,DIVF,ADD ,"
      "ADDI,ADDL,SUB ,SUBI,SUBL,MUL ,MULI,MULL,DIV ,DIVI,DIVL,DVU "
      ",DVUI,DVUL,"
      "MOD ,MODI,MODL,MDU ,MDUI,MDUL,AND ,ANDI,ANDL,OR  ,ORI ,ORL ,XOR "
      ",XORI,"
      "XORL,SHL ,SHLI,SHLL,SHR ,SHRI,SHRL,SRU ,SRUI,SRUL,EQ  ,EQF ,NE  "
      ",NEF ,"
      "LT  ,LTU ,LTF ,GE  ,GEU ,GEF ,BZ  ,BZF ,BNZ ,BNZF,BE  ,BEF ,BNE "
      ",BNEF,"
      "BLT ,BLTU,BLTF,BGE ,BGEU,BGEF,CID ,CUD ,CDI ,CDU ,CLI ,STI ,RTI "
      ",BIN ,"
      "BOUT,NOP ,SSP "
      ",PSHA,PSHI,PSHF,PSHB,POPB,POPF,POPA,IVEC,PDIR,SPAG,TIME,"
      "LVAD,TRAP,LUSP,SUSP,LCL ,LCA "
      ",PSHC,POPC,MSIZ,PSHG,POPG,NET1,NET2,NET3,"
      "NET4,NET5,NET6,NET7,NET8,NET9,POW ,ATN2,FABS,ATAN,LOG ,LOGT,EXP "
      ",FLOR,"
      "CEIL,HYPO,SIN ,COS ,TAN ,ASIN,ACOS,SINH,COSH,TANH,SQRT,FMOD,IDLE,";

// types and type masks. specific bit patterns and orderings needed by
// expr()
enum {
    CHAR = 1,
    SHORT,
...
void info_print_type_str(uint t)
{
    ...
    if (t & PMASK) {
        ...
        case ARRAY:
            ...
            fprintf(info_fd, "|%d",
                tsize(((array_t*)(va + (t >> TSHIFT)))->type));
...
void info_close(int text)
{
    ...
    if (debug) {
        fprintf(
            info_fd, "# .text 0x%08x (+0x%08x) 0x%08x\n", 0, text, text);
...
void err(char* msg)
{
    fprintf(stderr, "%s : [%s:%d] error: %s\n", cmd, file, line,
        msg); // XXX need errs to power past tokens (validate for each err
              // case.)
    if (++errs > 10) {
...
void emj(int i, int c)
{
    emi(i, c - ip - 4); // jump
}
void eml(int i, int c)
{
    emi(i, c - loc); // local
}
void emg(int i, int c)
{
    if (c < BSS_TAG) {
        *pdata++ = ip;
    } else {
    ...
    for (;;) {
        ...
        case '#':
            if (!memcmp(pos, "include", 7)) {
                if (ifile) {
                    err("can't nest include files");
                    exit(-1);
                } // include errors bail out otherwise it gets messy
                pos += 7;
                while (*pos == ' ' || *pos == '\t') {
                    pos++;
                }
                if (*pos != '"' && *pos != '<') {
                    err("bad include file name");
                    exit(-1);
                }
                ipos = pos++;
                if (*pos == '/') {
                    ...
                    memcpy(iname, incl, b = strlen(incl));
                    iname[b++] = '/';
                } else {
                    for (b = strlen(file); b; b--)
                        if (file[b - 1] == '/') {
                            memcpy(iname, file, b);
                            break;
                        }
        ...
        case '$':
            ...
            tk ^= (b = pos - p);
            while (id) {
                if (tk == id->hash && (b < 5 || !memcmp(id->name, p, b))) {
                    tk = id->tk;
                    return;
                } // b < 5 dependant on hash func and size
                id = id->next;
            }
            id = (ident_t*)vp;
            vp += sizeof(ident_t);
        ...
        case '0' ... '9':
            if (*pos == '.') {
                ...
                frac:
                    ...
                    if (*pos == 'f') {
                        pos++; // XXX should floats be treated different?
                    }
                    ty = DOUBLE;
                    tk = Numf; // XXX look into inferno/1e1src.tgz
                               // libk/charstod.c
                    return;
                }
            } else if (*pos == 'x' || *pos == 'X') {
            ...
            while ((b = *pos++) != tk) {
                if (b == '\\') {
                    ...
                    case 'x':
                        //            b = (*pos - '0') * 16 + pos[1] - '0';
                        //            pos += 2; // XXX
                        //            this is broke!!! 0xFF needs to become
                        //            -1 also
                        switch (*pos) {
                        case '0' ... '9':
                            b = *pos++ - '0';
                        ...
                        default:
                            b = 0;
                            pos++;
                            break; // XXX you can try a few in a reg c
                                   // compiler?!
                        }
                        switch (*pos) {
                        case '0' ... '9':
        ...
        case '.':
            ...
            return;
        // XXX eventually test for float? is this structure access x.y or
        // floating
        // point .5?
...
int imm() /// XXX move these back down once I validate prototypes working
          /// for
/// double immf()
{
    int *b = e, c;
...
double immf()
{
    ...
    if (*e == Num) {
        ...
        err("bad float constant expression");
        c = 0.0;
    }
    e = b;
    return c;
...
int tsize(uint t)
{ // XXX return unsigned? or error checking on size
    array_t* a;
    struct_t* s;
    switch (t & TMASK) {
    case ARRAY:
        a = (array_t*)(va + (t >> TSHIFT));
        return a->size * tsize(a->type);
    case STRUCT:
        if ((s = (struct_t*)(va + (t >> TSHIFT)))->align) {
            return s->size;
        }
...
{ // XXX return unsigned?
    if (t & PMASK) {
        ...
        return tsize(((array_t*)(va + (t >> TSHIFT)))
                         ->type); // XXX need to test this!
    else {
        return 1;
    }
}
...
uint basetype()
{
    ...
    case Struct:
        ...
        found:
            ...
            if (s->align) {
                err("struct or union redefinition");
            }
            next();
        } else {
            skip('{');
...
uint* type(uint* t, ident_t** v, uint bt)
{
    ...
    if (tk == Paren) {
        ...
        if (tk == ')') {
            if (v) {
                err("bad abstract function type");
            }
            next();
            *t |= FUN | ((vp - va) << TSHIFT);
            t = (uint*)vp;
    ...
    if (tk == Paren) { // function
        ...
        for (pt = p = 0; tk != ')'; p++) {
            ...
            if ((a = basetype())) {
                ...
                d = 0;
                type(&d, &n,
                    a); // XXX should accept both arg/non arg if v == 0  XXX
                // i.e. function declaration!
                if (d == FLOAT) {
                    d = DOUBLE; // XXX not ANSI
                }
            ...
            if (n) {
                ...
                if (*v == n) {
                    *v = (ident_t*)
                        ploc; // hack if function name same as parameter
                }
                ploc->class = n->class; // XXX make sure these are unwound
                                        // for abstract
                ...
                ploc++;
                if ((d & TMASK) == ARRAY)
                    d = ((array_t*)(va + (d >> TSHIFT)))->type
                        + PTR; // convert array to pointer
                n->local = 1;
                n->class = Auto;
                n->type = d;
                n->val = p * 8 + 8;
            ...
            if (tk == Comma) {
                next(); // XXX desparately need to flag an error if not a
                        // comma or close
            }
            // paren!
            if (tk == Dots) {
                next();
                if (tk != ')') {
                    err("expecting close parens after dots");
                }
                break;
            }
        }
        ...
        *(uint*)(vp + 4) = pt;
        ...
        while (tk == Brak) { // array
            ...
            a = 0;
            if (tk != ']') {
                // XXX need constant in vc...not tree!  I think this is
                // going go be a
                // bug unless you push vs,ty
                if ((int)(a = imm()) < 0) {
                    err("bad array size");
                }
            }
...
void decl(int bc)
{
    ...
    for (;;) {
        ...
        } // XXX typedef inside function?  probably bad!
        else {
            if (!(bt = basetype())) {
                if (bc == Auto) {
                    break;
        ...
        for (;;) {
            ...
            if (!v) {
                ...
                if (bc != Static || sc != Static) {
                    err("bad nested function");
                }
                if ((t & TMASK) != FUN) {
                    err("bad function definition");
                }
                rt = *(uint*)(va + (t >> TSHIFT));
                if (v->class == FFun) {
                    patch(v->val, ip);
                    ffun--;
                    if (rt != *(uint*)(va + (v->type >> TSHIFT))
                        || ((bt = *(uint*)(va + (v->type >> TSHIFT) + 4))
                               && bt != *(uint*)(va + (t >> TSHIFT) + 4))) {
                        err("conflicting forward function declaration");
                    }
                } else if (v->class) {
                    err("duplicate function definition");
                }
                v->class = Fun;
                v->type = t;
                v->val = ip;
                ...
                //        if (bc != Static || sc != Static) err("bad nested
                //        function
                //        declaration");
                if (v->class) {
                    err("duplicate function declaration");
                }
                v->class = FFun;
                v->type = t;
                ffun++;
                ...
                if (sc != Typedef) { // XXX typedefs local to functions?
                    if ((t & TMASK) == ARRAY) {
                        v->class = (sc == Auto)
                            ? Lea
                            : Leag; // not lvalue if array
                    }
                    size = tsize(t);
                    align = talign(t);
                    if (sc == Auto) {
                        v->val = loc
                            = (loc - size) & -align; // allocate stack space
                        if (tk == Assign) {
                            node(Auto, (int*)t, (int*)v->val);
                            b = e;
                            next();
                            expr(Cond);
                            cast(t < UINT ? INT : t);
                        ...
                        if (tk == Assign) {
                            v->val = data = (data + align - 1)
                                & -align; // allocate data space
                            hglo = data;
                            next();
                            if (tk == '"') {
                                if ((t & TMASK) != ARRAY) {
                                    err("bad string initializer");
                                }
                                next();
                                while (tk == '"') {
                                    next();
                                ...
                                if ((t & TMASK) != ARRAY) {
                                    err("bad array initializer");
                                }
                                next();
                                while (tk != '}') {
                                    switch (((array_t*)(va + (t >> TSHIFT)))
                                                ->type) {
                                    case UCHAR:
                                    case CHAR:
                                        *(char*)(gs + data) = imm();
                                ...
                                next();
                                if (size) {
                                    data = hglo + size * align; // XXX need
                                                                // to zero
                                                                // fill if
                                                                // size >
                                }
                                // initialized part  XXX but may
                                // default since using sbrk vs
                                // malloc?
...
void member(int stype, struct_t* s)
{
    int size, align, ssize = 0, salign = 1;
    uint bt, t;
    ident_t* v;
    member_t *m, **mp;
        ...
        for (;;) {
            ...
            if (!v) {
                ...
                *mp = m = (member_t*)vp;
                ...
                if (stype == Struct) {
                    ssize
                        = (m->offset = (ssize + align - 1) & -align) + size;
                } else if (size > ssize) {
                    ssize = size;
                }
                if (align > salign) {
...
void mul(int* b)
{ // XXX does this handle unsigned correctly?
    if (*b == Num) {
        if (*e == Num) {
            e[2] *= b[2];
            return;
...
void add(uint* b) // XXX make sure to optimize (a + 9 + 2) -> (a + 11) and
// (a + 9 - 2) -> (a + 7)
{
    if (*b == Num) {
        if (*e == Num || *e == Lea || *e == Leag) {
            e[2] += b[2];
            return;
        } // XXX  <<>> check
        if (!b[2]) {
    ...
    if (*e == Num) {
        if (*b == Lea) {
            ...
            *e = Lea;
            return;
        } // XXX structure offset optimizations
        if (*b == Leag) {
            e[2] += b[2];
            *e = Leag;
            return;
...
void addr()
{
    ...
    switch (*e) {
    case Fun:
        return; // XXX dont think ty is going to be right?
    case Leag:
    case Lea:
        return; // XXX
    case Static:
...
void trim()
{ // trim dead code from expression statements (just the common
    // cases)
    if (*e >= Cic && *e <= Cus) {
        e += 2; // remove conversion after assignment
    }
    if (*e == Add && *(int*)e[2] == Num) {
        e = (int*)e[1]; // convert x++ into ++x
    }
}
...
void expr(int lev)
{
    ...
    case Id:
        if (id->class) {
            node(id->class, (int*)(ty = id->type),
                (id->class == FFun) ? (int*)id : (int*)id->val);
            next();
            break;
        }
        id->class = FFun;
        ...
        *(uint*)vp = INT;
        ...
        if (tk != Paren) {
            ...
            fprintf(stderr,
                "%s : [%s:%d] warning: undeclared function called here\n",
                cmd, file, line);
        break;
    ...
    case Sizeof:
        ...
        if (t && (tt = basetype())) {
            ...
            expr(Dot);
            e = b;
        } // XXX backout any data seg allocs
        *(e -= 4) = Num;
        e[2] = tsize(ty);
        ty = INT;
        if (t) {
    ...
    while (tk >= lev) {
        ...
        case Cond:
            ...
            if (!((ty & PAMASK) && ((t & PAMASK) || t <= UINT))) {
                if ((t & PAMASK) && ty <= UINT) {
                    ...
                    err("bad conditional expression types");
                } else if (tt & FLOAT) {
                    dd = flot(dd, ty);
                    d = flot(d, t);
                    ty = DOUBLE;
        ...
        case Dot: // XXX do some optimization for x.y on stack or global,
                  // then work
            // on x.y.z (cause it wont be done in rval or lval)
            addr(); //  a.b --> (&a)->b --> *((&a) + b)
        case Arrow:
            ...
            for (m = ((struct_t*)(va + (ty >> TSHIFT)))->member; m;
                 m = m->next)
                if (m->id == id) {
                    goto found;
                }
        ...
        case Brak: // XXX these dont quite work when used with pointers?
                   // still?
            // test?
...
void lbf(int* b)
{
    ...
    switch (*b) {
    case Auto:
        eml(LBL + lmod(b[1]), b[2]);
        return;
    case Static:
        emg(LBG + lmod(b[1]), b[2]);
        return;
    case Numf:
        d = *(double*)(b + 2);
        if (((int)(d * 256.0) << 8 >> 8) / 256.0 == d)
            emi(LBIF, d * 256.0);
        else {
            data = (data + 7) & -8;
            *(double*)(gs + data) = d;
...
void opaf(int* a, int o, int comm)
{
    ...
    a = (int*)a[1];
    t = a[1] == FLOAT
        || a[1] == DOUBLE; // XXX need more testing before confident
    switch (*a) {
    case Auto: // loc fop= expr
        if (comm && t) {
            rv(b);
            eml(LBL + lmod(a[1]), a[2]);
        } else {
            lbf(b);
            eml(LL + lmod(a[1]), a[2]);
            if (!t) {
...
void opa(int* a, int o, int comm)
{
    ...
    switch (*a) {
    case Auto:
        if (*b == Num && b[2] << 8 >> 8 == b[2]) {
            eml(LL + lmod(a[1]), a[2]);
            emi(o + OPI, b[2]);
        } // loc op= num
        else if (*b == Auto && !lmod(b[1])) {
            eml(LL + lmod(a[1]), a[2]);
            eml(o + OPL, b[2]);
        } // loc op= locint
        else if (comm) {
    ...
    case Ptr:
        ...
        em(SX + smod(a[1])); // XXX many more (SX,imm) optimizations
                             // possible (here
        // and elsewhere)
        return;
    default:
...
int test(int* a, int t)
{
    ...
    case Gef:
        ...
        return emf(BGEF, t);
    case Lor:
        return test(a + 2, test((int*)a[1], t));
    case Lan:
        b = testnot((int*)a[1], 0);
        t = test(a + 2, t);
        patch(b, ip);
...
int testnot(int* a, int t)
{
    ...
    case Lor:
        ...
        return t;
    case Lan:
        return testnot(a + 2, testnot((int*)a[1], t));
    case Not:
        return test(a + 2, t);
    case Notf:
        rv(a + 2);
...
void rv(int* a)
{
    ...
    case Assign:
        ...
        case Ptr:
            ...
            case Auto:
                rv(b);
                eml(LBL + lmod(a[3]), a[4]);
                break; // *loc = expr // XXX all good? in the presense of
                       // multiple
            // casts?
            case Static:
                rv(b);
                emg(LBG + lmod(a[3]), a[4]);
                break; // *glo = expr
            default:
                rv(a + 2);
                switch (*b) {
    ...
    case Fcall:
        ...
        a = (int*)a[1];
        for (t = 0; b; t += 8) {
            if (b[1] == DOUBLE || b[1] == FLOAT) {
                rv(b + 2);
                loc -= 8;
                em(PSHF);
            } else if (b[2] == Num && b[4] << 8 >> 8 == b[4]) {
                loc -= 8;
                emi(PSHI, b[4]);
            } else {
                rv(b + 2);
...
void stmt()
{
    ...
    case Switch:
        ...
        if (es == e) { // err("no case in switch statement");   XXX
        ...
        } else { // XXX lots of possible signed/unsigned under/overflow
                 // issues in
            // this block
            cmin = cmax = *(et = es - 2);
            while (et > e) {
            ...
            et = es;
            if ((uint)(cmax - cmin)
                <= ((int)es - (int)e) * 2) { // jump table
                if (cmin > 0 && cmax <= (int)es - (int)e) {
                    cmin = 0;
                } else if (cmin) {
                    opi(SUB, cmin);
                    cmax -= cmin;
    ...
    case Id:
        if (*pos == ':') {
            ...
            if (!id->class) {
                ...
                err("bad label name"); // XXX should let labels override
                                       // globals just
            }
            // like other locals
            next();
...
int main(int argc, char* argv[])
{
    ...
    while (--argc && *file == '-') {
        ...
        usage:
            fprintf(stderr,
                "usage: %s [-v] [-s] [-Ipath] -o exefile file ...\n", cmd);
            return -1;
        }
        file = *++argv;
    }
    ...
    pos = "asm auto break case char continue default do double else enum "
          "float "
          "for goto if int long return short "
          "sizeof static struct switch typedef union unsigned void while "
          "va_list "
          "va_start va_arg main";
    for (i = Asm; i <= Va_arg; i++) {
        next();
        id->tk = i;
    }
    next();
    ...
    if (text + data + bss > SEG_SZ) {
        err("text + data + bss segment exceeds maximum size");
    }
    if (!(amain = tmain->val)) {
    ...
    if (verbose || errs) {
        fprintf(
            stderr, "%s : %s compiled with %d errors\n", cmd, file, errs);
    }
    if (verbose)
