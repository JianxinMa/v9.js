...
passifstat:
  ...
  case HALT:
    if (user || verbose)
      dprintf(2, "halt(%d) cycle = %u\n", a,
              cycle + (int)((uint)xpc - xcycle) / 4);
    goto finish; // XXX should be supervisor!
  case IDLE:
    if (user) {
      trap = FPRIV;
      break;
  ...
  case LBA:
    b = a;
    goto loopstart; // XXX need LAB, LAC to improve k.c  // or maybe a = a * imm
                    // + b
                    // ?  or b = b * imm + a ?
  case LCA:
    c = a;
  ...
  case CYC:
    ...
    goto loopstart; // XXX protected?  XXX also need wall clock time instruction
  case MSIZ:
    if (user) {
      trap = FPRIV;
      break;
  ...
  case TIME:
    ...
    timeout = a;
    goto loopstart; // XXX cancel pending interrupts if disabled?
  ...
  case NET2:
    ...
    a = close(a);
    goto loopstart; // XXX does this block?
  case NET3:
    if (user) {
      trap = FPRIV;
      break;
