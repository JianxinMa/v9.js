...
function flush() {
    ...
    while (tpages) {
        tpages = us(tpages - 1);
        v = us(tpage.readUInt32LE(us(tpages * 4)));
...
function cpu(pc, sp) {
    ...
    interrupt = function() {
        ...
        xsp = us(xsp - 8);
        p = us(tw.readUInt32LE(us((xsp >>> 12) * 4)));
        if (!p) {
            p = us(wlook(xsp));
            if (!p) {
        ...
        xsp = us(xsp - 8);
        p = us(tw.readUInt32LE(us(us(xsp >>> 12) * 4)));
        if (!p) {
            p = us(wlook(xsp));
            if (!p) {
                console.log("kstack fault\n");
    ...
    fixsp = function() {
        ...
        v = us(xsp - tsp);
        p = us(tw.readUInt32LE(us((v >>> 12) * 4)));
        if (p) {
            xsp = us(v ^ us(p - 1));
            tsp = us(xsp - v);
    ...
    fixpc = function() {
        ...
        v = us(xpc - tpc);
        p = us(tr.readUInt32LE(us((v >>> 12) * 4)));
        if (!p) {
            p = us(rlook(v));
            if (!p) {
                trap = us(FIPAGE);
    ...
    decode = function() {
        ...
        switch (us(ir & 0xFF)) {
            ...
            case MCPY:
                while (c) {
                    t = us(tr.readUInt32LE(us((b >>> 12) * 4)));
                    if (!t) {
                        t = us(rlook(b));
                        if (!t) {
                            follower = exception;
                    ...
                    p = us(tw.readUInt32LE(us((a >>> 12) * 4)));
                    if (!p) {
                        p = us(wlook(a));
                        if (!p) {
                            follower = exception;
            ...
            case MCMP:
                while (true) {
                    ...
                    p = us(tr.readUInt32LE(us((a >>> 12) * 4)));
                    if (!p) {
                        p = us(rlook(a));
                        if (!p) {
                            follower = exception;
                    ...
                    p = us(a ^ (p & -2));
                    t = us(b ^ (t & -2));
                    t = us(mem.slice(p, p + u).compare(mem.slice(t, t + u)));
                    if (t) {
                        a = us(t);
                        b = us(b + us(c));
                        c = us(0);
            ...
            case MCHR:
                while (true) {
                    ...
                    v = us(a ^ (p & -2));
                    t = us(mem.slice(v, v + u).indexOf(b));
                    if (t !== -1) {
                        a = us(a + us(t));
                        c = us(0);
                        break;
            ...
            case LEV:
                if (ir < fsp) {
                    t = us(mem.readUInt32LE(xsp + (ir >> 8)) + tpc);
                    fsp = us(fsp - ((ir + 0x800) & -256));
                } else {
                    v = us(xsp - tsp + (ir >> 8));
                    p = us(tr.readUInt32LE(us((v >>> 12) * 4)));
                    if (!p) {
                    ...
                    t = us(mem.readUInt32LE((v ^ p) & -8) + tpc);
                    fsp = us(0);
                }
            ...
            case POPA:
                ...
                a = us(mem.readUInt32LE((v ^ p) & -8));
                xsp = us(xsp + us(8));
                follower = fixsp;
                return;
            case POPB:
                if (fsp) {
                    b = us(mem.readUInt32LE(xsp));
                    xsp = us(xsp + us(8));
                    fsp = us(fsp - 8) << 8;
                    follower = chkpc;
                    return;
                ...
                b = us(mem.readUInt32LE((v ^ p) & -8));
                xsp = us(xsp + us(8));
                follower = fixsp;
                return;
            case POPC:
                if (fsp) {
                    c = us(mem.readUInt32LE(xsp));
                    xsp = us(xsp + us(8));
                    fsp = us(fsp - 8) << 8;
                    follower = chkpc;
                    return;
            ...
            case LL:
                if (ir < fsp) {
                    a = us(mem.readUInt32LE(xsp + (ir >> 8)));
                    follower = chkpc;
                    return;
                }
                v = us(xsp - tsp + (ir >> 8));
            ...
            case LLH:
                if (ir < fsp) {
                    a = us(mem.readUInt16LE(xsp + (ir >> 8)));
                    follower = chkpc;
                    return;
                }
                v = us(xsp - tsp + (ir >> 8));
            ...
            case LLC:
                ...
                a = us(mem.readInt8(v ^ p & -2)); // & precedes ^
                if (fsp || (v ^ (xsp - tsp)) & -4096) {
                    follower = chkpc;
                    return;
                }
            ...
            case LBL:
                if (ir < fsp) {
                    b = us(mem.readUInt32LE(xsp + (ir >> 8)));
                    follower = chkpc;
                    return;
                }
                v = us(xsp - tsp + (ir >> 8));
            ...
            case LBLH:
                if (ir < fsp) {
                    b = us(mem.readUInt16LE(xsp + (ir >> 8)));
                    follower = chkpc;
                    return;
                }
                v = us(xsp - tsp + (ir >> 8));
            ...
            case LCL:
                if (ir < fsp) {
                    c = us(mem.readUInt32LE(xsp + (ir >> 8)));
                    follower = chkpc;
                    return;
                }
                v = us(xsp - tsp + (ir >> 8));
            ...
            case MULL:
                if (ir < fsp) {
                    a = us(a * mem.readInt32LE(xsp + (ir >> 8)));
                    follower = chkpc;
                    return;
                }
                v = us(xsp - tsp + (ir >> 8));
            ...
            case DIVL:
                if (ir < fsp) {
                    t = us(mem.readUInt32LE(xsp + (ir >> 8)));
                    if (!t) {
                        trap = us(FARITH);
                        break;
                    }
            ...
            case NOP:
                ...
                return;
            case CYC:
                a = us(cycle + Math.floor((xpc - xcycle) / 4));
                follower = chkpc;
                return;
            case MSIZ:
                if (user) {
            ...
            case RTI:
                ...
                pc = us(mem.readUInt32LE((xsp ^ p) & -8) + tpc);
